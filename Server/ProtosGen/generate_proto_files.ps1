...
$gameClientProtosPath = "../../Client/ActionRpg.Client.GameClient/Protos"
...
foreach ($file in $files) {
	...
	# Write the new proto
    $clientGen = "./protoc.exe -I . --csharp_out=$gameClientProtosPath --grpc_out=$gameClientProtosPath --plugin=protoc-gen-grpc=grpc_csharp_plugin.exe " +$file.Name
